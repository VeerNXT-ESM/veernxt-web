 ...
 * Supabase; writes only new files, never touches the database.
 *
 * Run this before any other step of the rearchitecture, and re-run it
 * before any future step that touches these tables.
 *
 * Usage: node scripts/backup_content_tables.mjs [--out DIR]
 */
...
async function main() {
  ...
  const outDir = opts.outDir || path.join('K:', 'tmp', 'db_backups', timestamp);

...
import {
  ...
  Palette, Type, LayoutGrid, RefreshCw, CheckCircle2,
} from 'lucide-react';
...
// Dark-mode override for surface/text tokens only -- brand/semantic accent
// colors stay as the theme defines them, only the page's own light/dark
// surface flips, same idea as every other light/dark toggle in this app.
...
const ThemeEditor = () => {
  ...
  const [theme, setTheme] = useState(null); // { id, name, description, tokens }
  ...
  // Real chapter content for the live preview -- pulled from an actual
  // Published blocks-format resource, not fabricated sample copy.
      ...
      try {
        ...
        if (resource?.storage_base_url) {
          ...
          if (res.ok) {
            ...
            setPreviewBlocks(Array.isArray(chapterData.blocks) ? chapterData.blocks : []);
            setPreviewTitle(chapterData.title || resource.title);
          }
        }
      } catch (err) {
        console.warn('Could not load a real chapter for the theme preview:', err.message);
      } finally {
  ...
  // Apply the in-progress (unsaved) token edits straight to the preview
  // root -- this intentionally bypasses ReaderThemeProvider's id-based
  // resolution, since what's being previewed here is an arbitrary draft
  // that may not be saved (or even have an id) yet.
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Editing a system preset (e.g. "academic") always creates a new
      // custom row with a real UUID rather than mutating the built-in
      // preset -- swap the URL to that id so a refresh reloads the saved
      // customization instead of reverting to the pristine system theme.
    ...
    <div className="te-root">
      <header className="te-topbar">
        <div className="te-topbar-left">
          <button className="te-back-btn" onClick={() => navigate('/admin/reader-themes')}>
            <ChevronLeft size={18} />
          </button>
          <div>
            <div className="te-topbar-title-row">
              ...
              {isSystemTheme && <span className="te-system-badge">Built-in — saving creates a copy</span>}
            </div>
            <input
        ...
        <div className="te-topbar-right">
          <div className="te-viewport-toggle">
            {Object.entries(VIEWPORTS).map(([key, v]) => {
              const Icon = v.icon;
              return (
                <button key={key} className={`te-viewport-btn ${viewport === key ? 'active' : ''}`} onClick={() => setViewport(key)} title={v.label}>
                  <Icon size={15} />
                </button>
              );
            })}
          </div>
          <button className="te-mode-toggle" onClick={() => setDarkPreview((v) => !v)}>
            {darkPreview ? <Moon size={15} /> : <Sun size={15} />} {darkPreview ? 'Dark' : 'Light'}
          </button>
          <button className="te-btn" onClick={handleReset}><RotateCcw size={15} /> Reset</button>
      ...
      <div className="te-body">
          ...
          <div className="te-preview-frame" style={{ width: VIEWPORTS[viewport].width }}>
            ...
            >
              {previewLoading ? (
                ...
                <p className="te-preview-empty">No published book content available to preview yet.</p>
              )}
            </div>
          </div>
        </main>
        ...
        <aside className="te-controls-panel">
          {activePanel === 'colors' && (
            <>
              <div className="te-controls-group">
                <div className="te-controls-group-header"><h4>Brand</h4></div>
                {colorField('Primary', tokens.primary, (v) => updateToken('primary', v))}
                {colorField('Secondary', tokens.secondary, (v) => updateToken('secondary', v))}
              </div>
              <div className="te-controls-group">
                <div className="te-controls-group-header"><h4>Reader</h4></div>
                {colorField('Background', tokens.background, (v) => updateToken('background', v))}
                {colorField('Surface', tokens.surface, (v) => updateToken('surface', v))}
                {colorField('Text', tokens.text, (v) => updateToken('text', v))}
                {colorField('Muted Text', tokens.textMuted, (v) => updateToken('textMuted', v))}
                {colorField('Heading', tokens.heading, (v) => updateToken('heading', v))}
                {colorField('Border', tokens.border, (v) => updateToken('border', v))}
              </div>
              <div className="te-controls-group">
                <div className="te-controls-group-header"><h4>Semantic</h4></div>
                {colorField('Success', tokens.success, (v) => updateToken('success', v))}
                {colorField('Warning', tokens.warning, (v) => updateToken('warning', v))}
                {colorField('Danger', tokens.danger, (v) => updateToken('danger', v))}
                {colorField('Info', tokens.info, (v) => updateToken('info', v))}
              </div>
              <p className="te-controls-hint">
                Callouts (Important, Exam Tip, Definition) and tables reuse these same colors — Danger, Warning, Info
                and Primary respectively — so changing them here restyles every block at once.
              </p>
            </>
          )}
          ...
          {activePanel === 'typography' && (
            <div className="te-controls-group">
              <div className="te-controls-group-header"><h4>Fonts</h4></div>
              ...
              <div className="te-select-field">
                <label>Drop Cap Font</label>
                <select value={tokens.dropcapFont} onChange={(e) => updateToken('dropcapFont', e.target.value)}>
                  {FONT_OPTIONS.map((f) => <option key={f.value} value={f.value}>{f.label}</option>)}
                </select>
              </div>

              <div className="te-controls-group-header" style={{ marginTop: '0.5rem' }}><h4>Body</h4></div>
              {sliderField('Size', tokens.bodySize, 'rem', 0.9, 1.4, 0.025, (v) => updateToken('bodySize', v))}
              {sliderField('Line Height', tokens.bodyLineHeight, '', 1.4, 2.1, 0.05, (v) => updateToken('bodyLineHeight', v))}

              <div className="te-controls-group-header" style={{ marginTop: '0.5rem' }}><h4>Headings</h4></div>
              ...
              {sliderField('Weight', tokens.headingWeight, '', 400, 900, 100, (v) => updateToken('headingWeight', v))}
            </div>
          )}

          {activePanel === 'layout' && (
            <div className="te-controls-group">
              <div className="te-controls-group-header"><h4>Reading Width</h4></div>
              {sliderField('Reading Width', tokens.readingWidth, 'px', 560, 900, 10, (v) => updateToken('readingWidth', v))}
              {sliderField('Content Width', tokens.contentWidth, 'px', 800, 1400, 20, (v) => updateToken('contentWidth', v))}
              ...
              {sliderField('Small', tokens.radiusSm, 'px', 0, 20, 1, (v) => updateToken('radiusSm', v))}
              {sliderField('Medium', tokens.radiusMd, 'px', 0, 24, 1, (v) => updateToken('radiusMd', v))}
              {sliderField('Large', tokens.radiusLg, 'px', 0, 32, 1, (v) => updateToken('radiusLg', v))}
            </div>
          )}
        </aside>
      </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .te-body { flex: 1; display: grid; grid-template-columns: 200px 1fr 320px; min-height: 0; }

        .te-side-nav { border-right: 1px solid rgba(255,255,255,0.08); padding: 1rem 0.75rem; display: flex; flex-direction: column; gap: 0.2rem; overflow-y: auto; }
        .te-side-nav-item { display: flex; align-items: center; gap: 0.6rem; background: none; border: none; border-radius: 8px; padding: 0.6rem 0.7rem; font-size: 0.85rem; font-weight: 600; color: #8b949e; cursor: pointer; text-align: left; }

...
- `9284eeb` — `lc_exam_resource_map` + mapping script, conducting-body logos + admin page, `resources_v2.format` + migration script (first pass, old chrome still wrapping new content), `Resources_Migration_Plan.md`.
- `eabb728` — the sidebar-layout reader chrome fix (§29.5's "process note").
...
### 29.8 Next steps — user is explicitly waiting, do not act speculatively

1. **`scripts/map_exam_resources_gemini.mjs --execute`** (§29.3) — check if the background run finished; if it errored out partway, it's safe to re-run without `--all` (skips exams already in `lc_exam_resource_map`). Once done: wire `useExamContent.js` to actually read from the new table (currently unused by the app), and consider building the admin "low-confidence rows" review view the original plan called for.
2. **Wait for the content team** to finish more enriched books + get further along on PYQs (`FINAL_PYPS_STRUCTURED`, 68 files as of this session — has a text-encoding mojibake bug, `Ã·`/`â€“` instead of `÷`/`–`, needs fixing at the source before ingestion) before re-running `scripts/migrate_resources_to_blocks.mjs`. The user wants to decide then whether PYQs reuse `BlockRenderer` or need their own quiz-paper view — genuinely open, don't assume.
3. **Jobs, not started**: admin `/admin/jobs` currently just wraps the candidate-facing `JobBoard.jsx` (light theme) instead of using the admin CMS's own dark `lc-*` conventions like every other admin page — needs a dedicated dark-themed table. The visible "INDIAGOVTEXAM" conducting-body values are the scraper's source-site name leaking in as a placeholder — confirms the still-unbuilt Phase 2 from this session's earlier admin plan (`jobs.conducting_body_id` + Gemini resolution + tag fallback) is the real fix, not a display bug. Scraper re-run (`scrape_to_json.js` → `push_json_to_supabase.js`) was discussed but not run this session — confirm with the user immediately before the production purge/insert step when it happens.
